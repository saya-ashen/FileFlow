...
const fetchData = async (dirID) => {
    try {
        const params = dirID ? { item_id: dirID } : {};
        ...
        currentFolder.value = items.filter(item => item.name === '.');
        tableData.value = items.filter(item => item.name !== '.');
    } catch (error) {
...
onMounted(async () => {
    ...
    await fetchData(dirID);
    /*router.push({
      path: '/welcome',
      query: {
        dirID: currentFolder.value[0].id
      }
    });*/
});
/*watch(() => route.query.dirID, async () => {
  await fetchData(route.query.dirID);
...
const handleDownload = async () => {
    ...
    // 获取所有选中文件的ID
    const selectedFiles = multipleSelection.value.map(item => item.id);
    ...
    try {
        ...
        //const response = await axios.get(baseUrlApi("/api/download"), { headers: { 'Authorization': 'Bearer ' + getToken().access_Token }, params: { files: selectedFiles }, responseType: 'blob' });
        ...
        const defaultFileName = isSingleFile ? multipleSelection.value[0].name : 'download.zip';
...
const handleUpload = async () => {
    ...
    input.onchange = async () => {
        ...
        try {
            const params = {
                folder_id: currentFolder.value[0].id,
            }
            // 发送请求到您的后端服务
            const response = await http.request<FilesResult>("post", baseUrlApi("/api/upload"),
                { data: formData, params: params, headers: { 'Content-Type': 'multipart/form-data' } });
